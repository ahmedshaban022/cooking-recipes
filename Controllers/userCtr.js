...
const register=async(req,res)=>{
    try {
        ...
        if(!name || !email || !password)return res.status(400).json({msg:"missing feilds!"})
...
const login= async(req,res)=>{
    try {
        ...
        if(!email || !password)return res.status(400).json({msg:"missing feilds!"})
        
        const user= await Users.findOne({email});
        if(!user) return res.status(400).json({msg:"Email or password is wrong"});

        const isMatched= await user.matchPassword(password);
        if(!isMatched)return res.status(400).json({msg:"Email or password is wrong"});
